...
const test_data_generator_1 = require("../../src/utils/test-data-generator");
test_1.test.describe('Registration Performance Tests', () => {
    let registrationPage;
    test_1.test.beforeEach(async ({ page }) => {
        registrationPage = new registration_page_1.RegistrationPage(page);
    });
    test_1.test.describe('Load Performance Tests', () => {
        (0, test_1.test)('REG_PERF_001: Page load performance under 5 seconds', async ({ page }) => {
        ...
        (0, test_1.test)('REG_PERF_002: Form submission performance', async () => {
            await registrationPage.navigateToRegistrationPage();
            const validUser = test_data_generator_1.TestDataGenerator.generateValidUser();
    ...
    test_1.test.describe('Bulk Registration Tests', () => {
        (0, test_1.test)('REG_PERF_003: Sequential bulk user registration', async () => {
            const userCount = 10;
            const users = test_data_generator_1.TestDataGenerator.generatePerformanceTestUsers(userCount);
            const results = [];
        ...
        - Min time: ${minTime}ms`);
            ...
            (0, test_1.expect)(successfulRegistrations.length).toBeGreaterThan(userCount * 0.8); // 80% success rate
            (0, test_1.expect)(averageTime).toBeLessThan(8000); // Average under 8 seconds
            (0, test_1.expect)(maxTime).toBeLessThan(15000); // Max under 15 seconds
        });
    });
    test_1.test.describe('Concurrent Registration Tests', () => {
        (0, test_1.test)('REG_PERF_004: Concurrent user registration', async ({ browser }) => {
            const concurrentUsers = 5;
            const users = test_data_generator_1.TestDataGenerator.generateConcurrentTestUsers(concurrentUsers);
            const registrationPromises = users.map(async (user, index) => {
                const context = await browser.newContext();
                const page = await context.newPage();
                const regPage = new registration_page_1.RegistrationPage(page);
                const startTime = Date.now();
                try {
        ...
        - Average individual time: ${averageTime.toFixed(2)}ms`);
            ...
            (0, test_1.expect)(successfulRegistrations.length).toBeGreaterThan(concurrentUsers * 0.6); // 60% success rate for concurrent
            (0, test_1.expect)(totalTime).toBeLessThan(20000); // Total time under 20 seconds
            (0, test_1.expect)(averageTime).toBeLessThan(12000); // Average individual time under 12 seconds
        });
    });
    test_1.test.describe('Memory and Resource Tests', () => {
        (0, test_1.test)('REG_PERF_005: Memory usage during form interactions', async ({ page }) => {
            await registrationPage.navigateToRegistrationPage();
            ...
            for (let i = 0; i < 10; i++) {
                const user = test_data_generator_1.TestDataGenerator.generateValidUser();
        ...
        (0, test_1.test)('REG_PERF_006: Network performance monitoring', async ({ page }) => {
        ...
        - Slow requests (>3s): ${slowRequests.length}`);
            ...
            (0, test_1.expect)(averageResponseTime).toBeLessThan(2000); // Average response time under 2 seconds
            (0, test_1.expect)(slowRequests.length).toBeLessThan(completedRequests.length * 0.1); // Less than 10% slow requests
        });
    });
    test_1.test.describe('Stress Testing', () => {
        (0, test_1.test)('REG_PERF_007: Form field stress test', async () => {
            await registrationPage.navigateToRegistrationPage();
            const startTime = Date.now();
            ...
            const finalUser = test_data_generator_1.TestDataGenerator.generateValidUser();
            await registrationPage.registerUser(finalUser);
            await registrationPage.verifySuccessfulRegistration();
            // Stress test should complete within reasonable time
            (0, test_1.expect)(totalTime).toBeLessThan(30000); // 30 seconds for 50 iterations
        });
    });
    test_1.test.describe('Browser Performance Tests', () => {
        (0, test_1.test)('REG_PERF_008: Performance across different viewport sizes', async ({ page }) => {
            const viewports = [
          ...
          - Total time: ${loadTime + formTime}ms`);
            ...
            (0, test_1.expect)(maxLoadTime).toBeLessThan(8000); // Load time under 8 seconds on any viewport
            (0, test_1.expect)(maxFormTime).toBeLessThan(12000); // Form submission under 12 seconds on any viewport
        });
    });
});
//# sourceMappingURL=registration-performance.spec.js.map