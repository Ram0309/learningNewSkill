...
base_test_1.test.describe('Performance Load Testing', () => {
    (0, base_test_1.performanceTest)('should handle homepage load under normal conditions @performance @load', async ({ performancePage, performance, logger }) => {
        ...
        (0, base_test_1.expect)(loadTime).toBeLessThan(3000); // Page should load within 3 seconds
        (0, base_test_1.expect)(webVitals.lcp).toBeLessThan(2500); // LCP should be under 2.5s
        (0, base_test_1.expect)(webVitals.fcp).toBeLessThan(1800); // FCP should be under 1.8s
        (0, base_test_1.expect)(webVitals.cls).toBeLessThan(0.1); // CLS should be under 0.1
        // Memory usage check
        ...
        if (memoryUsage) {
            ...
            (0, base_test_1.expect)(memoryUsagePercent).toBeLessThan(50); // Memory usage should be under 50%
        }
    ...
    (0, base_test_1.performanceTest)('should handle API response times under load @performance @api', async ({ api, performance, logger }) => {
        ...
        for (const endpoint of apiEndpoints) {
            ...
            const concurrentRequests = Array.from({ length: 50 }, () => api.get(endpoint, {
                timeout: 10000,
                captureMetrics: true
            }));
            const startTime = Date.now();
            ...
            // Performance assertions
            (0, base_test_1.expect)(successRate).toBeGreaterThan(95); // 95% success rate minimum
            (0, base_test_1.expect)(endTime - startTime).toBeLessThan(30000); // All requests within 30 seconds
            // Check individual response times
            const successfulResponses = responses
                .filter(r => r.status === 'fulfilled')
                .map(r => r.value);
            for (const response of successfulResponses) {
                (0, base_test_1.expect)(response.status()).toBeLessThan(400);
            }
    ...
    (0, base_test_1.performanceTest)('should handle database query performance @performance @database', async ({ db, performance, logger }) => {
        ...
        for (const queryTest of queryTests) {
            ...
            // Performance assertions
            (0, base_test_1.expect)(avgTime).toBeLessThan(queryTest.maxTime);
            (0, base_test_1.expect)(maxTime).toBeLessThan(queryTest.maxTime * 2); // Max time shouldn't be more than 2x expected
    ...
    (0, base_test_1.performanceTest)('should handle memory usage during extended operations @performance @memory', async ({ performancePage, performance, logger }) => {
        ...
        // Memory assertions
        (0, base_test_1.expect)(memoryIncreasePercent).toBeLessThan(200); // Memory shouldn't increase by more than 200%
        (0, base_test_1.expect)(finalMemory).toBeLessThan(100 * 1024 * 1024); // Final memory should be under 100MB
        // Check for memory leaks (memory should not continuously increase)
        const memoryTrend = memorySnapshots.slice(-3); // Last 3 snapshots
        const isIncreasing = memoryTrend.every((snapshot, index) => index === 0 || snapshot.memory > memoryTrend[index - 1].memory);
        if (isIncreasing) {
    ...
    (0, base_test_1.performanceTest)('should handle concurrent user simulation @performance @stress', async ({ performance, logger }) => {
        ...
        // Performance assertions
        (0, base_test_1.expect)(successRate).toBeGreaterThan(90); // 90% success rate minimum
        (0, base_test_1.expect)(endTime - startTime).toBeLessThan(60000); // All users complete within 60 seconds
    ...
    base_test_1.test.afterEach(async ({ performance, logger }) => {
...
//# sourceMappingURL=load-testing.performance.spec.js.map