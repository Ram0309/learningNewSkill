...
base_test_1.test.describe('Mobile App Navigation', () => {
    base_test_1.test.beforeEach(async ({ mobilePage }) => {
    ...
    (0, base_test_1.mobileTest)('should navigate through main app sections @mobile @smoke', async ({ mobilePage, logger }) => {
        logger.info('Testing main app navigation on mobile');
        // Verify home screen is loaded
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="home-screen"]')).toBeVisible();
        // Test bottom navigation
        const bottomNav = mobilePage.locator('[data-testid="bottom-navigation"]');
        await (0, base_test_1.expect)(bottomNav).toBeVisible();
        // Navigate to Profile section
        await mobilePage.tap('[data-testid="nav-profile"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="profile-screen"]')).toBeVisible();
        // Navigate to Settings section
        await mobilePage.tap('[data-testid="nav-settings"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="settings-screen"]')).toBeVisible();
        // Navigate to Search section
        await mobilePage.tap('[data-testid="nav-search"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="search-screen"]')).toBeVisible();
        // Return to Home
        await mobilePage.tap('[data-testid="nav-home"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="home-screen"]')).toBeVisible();
        logger.info('Main app navigation test completed');
    });
    (0, base_test_1.mobileTest)('should handle touch gestures @mobile @gestures', async ({ mobilePage, logger }) => {
        ...
        await (0, base_test_1.expect)(carousel).toBeVisible();
        // Get initial image
        ...
        (0, base_test_1.expect)(nextImage).not.toBe(initialImage);
        ...
        (0, base_test_1.expect)(previousImage).toBe(initialImage);
        // Test pull-to-refresh
        const contentArea = mobilePage.locator('[data-testid="content-area"]');
        await contentArea.swipe('down', { startY: 100, endY: 300 });
        // Verify refresh indicator appears
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="refresh-indicator"]')).toBeVisible();
        // Wait for refresh to complete
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="refresh-indicator"]')).not.toBeVisible({ timeout: 5000 });
        logger.info('Touch gestures test completed');
    });
    (0, base_test_1.mobileTest)('should work in portrait and landscape modes @mobile @orientation', async ({ mobilePage, logger }) => {
        logger.info('Testing orientation changes');
        // Start in portrait mode
        await mobilePage.setViewportSize({ width: 375, height: 812 });
        // Verify portrait layout
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="portrait-layout"]')).toBeVisible();
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="landscape-layout"]')).not.toBeVisible();
        // Switch to landscape mode
        await mobilePage.setViewportSize({ width: 812, height: 375 });
        // Wait for layout change
        await mobilePage.waitForTimeout(1000);
        // Verify landscape layout
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="landscape-layout"]')).toBeVisible();
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="portrait-layout"]')).not.toBeVisible();
    ...
    (0, base_test_1.mobileTest)('should handle form input on mobile @mobile @forms', async ({ mobilePage, testData, logger }) => {
        ...
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="date-picker"]')).toBeVisible();
        // Select date
        ...
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="country-dropdown"]')).toBeVisible();
        await mobilePage.tap('[data-testid="country-usa"]');
        // Submit form
        await mobilePage.tap('[data-testid="save-button"]');
        // Verify success message
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="success-message"]')).toBeVisible();
        logger.info('Mobile form input test completed');
    });
    (0, base_test_1.mobileTest)('should handle offline functionality @mobile @offline', async ({ mobilePage, logger }) => {
        ...
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="search-results"]')).toBeVisible();
        // Go offline
        await mobilePage.context().setOffline(true);
        // Verify offline indicator
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="offline-indicator"]')).toBeVisible();
        // Test cached content is still available
        await mobilePage.tap('[data-testid="nav-home"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="home-screen"]')).toBeVisible();
        // Test offline message for new requests
        await mobilePage.tap('[data-testid="nav-profile"]');
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="offline-message"]')).toBeVisible();
        // Go back online
        await mobilePage.context().setOffline(false);
        // Verify online indicator
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="offline-indicator"]')).not.toBeVisible();
        // Verify content loads normally
        await mobilePage.reload();
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="profile-screen"]')).toBeVisible();
        logger.info('Offline functionality test completed');
    });
    (0, base_test_1.mobileTest)('should handle push notifications @mobile @notifications', async ({ mobilePage, logger }) => {
        ...
        // Verify success message
        await (0, base_test_1.expect)(mobilePage.locator('[data-testid="settings-saved-message"]')).toBeVisible();
        logger.info('Push notifications test completed');
    });
    (0, base_test_1.mobileTest)('should handle app state changes @mobile @lifecycle', async ({ mobilePage, logger }) => {
        ...
        for (const screen of screens) {
            ...
            await (0, base_test_1.expect)(mobilePage.locator(`[data-testid="${screen}-screen"]`)).toBeVisible();
    ...
    (0, base_test_1.mobileTest)('should work with different screen sizes @mobile @responsive', async ({ mobilePage, logger }) => {
        ...
        for (const size of screenSizes) {
            ...
            await (0, base_test_1.expect)(mobilePage.locator('[data-testid="app-container"]')).toBeVisible();
            await (0, base_test_1.expect)(mobilePage.locator('[data-testid="bottom-navigation"]')).toBeVisible();
            ...
            (0, base_test_1.expect)(fontSizeNum).toBeGreaterThanOrEqual(14); // Minimum readable font size
            // Return to home for next iteration
            await mobilePage.tap('[data-testid="nav-home"]');
        }
        logger.info('Screen sizes test completed');
    });
    base_test_1.test.afterEach(async ({ mobilePage, logger }) => {
        // Capture screenshot for mobile test results
        await mobilePage.screenshot({
            path: `test-results/mobile-screenshots/${base_test_1.test.info().title.replace(/[^a-zA-Z0-9]/g, '_')}.png`,
            fullPage: true
        });
...
//# sourceMappingURL=app-navigation.mobile.spec.js.map