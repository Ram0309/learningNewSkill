import { test, expect, securityTest } from '../../src/core/base-test';

test.describe('Security Testing Suite', () => {

  securityTest('should prevent SQL injection attacks @security @injection', async ({ 
    api, 
    security, 
    logger 
  }) => {
  ...
  securityTest('should prevent XSS attacks @security @xss', async ({ 
    page, 
    security, 
    logger 
  }) => {
  ...
  securityTest('should enforce proper authentication @security @auth', async ({ 
    api, 
    security, 
    logger 
  }) => {
